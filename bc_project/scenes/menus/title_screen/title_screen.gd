...
func setup_title_screen() -> void:
	...
	%AuthorCreditLabel.parse_bbcode("[font_size=%s][color=#%s]%s: %s" % [SettingsController.scale_font_size(24),Global.color_TextBright.to_html(),tr("AUTHOR_CREDIT_LABEL"), CreditsResource.author])
	%LeadCreditLabel.parse_bbcode("[font_size=%s][color=#%s]%s: %s" % [SettingsController.scale_font_size(24),Global.color_TextBright.to_html(),tr("LEAD_CREDIT_LABEL"), CreditsResource.lead])

	%TitleAnimation.texture.pause = true
	%TitleAnimation.texture.current_frame = 0
	%TitleLabel.modulate = Color.TRANSPARENT
	%TitleLabel.parse_bbcode("[font_size=%s][color=#%s]%s" % [SettingsController.scale_font_size(42),Global.color_TextHighlight.to_html(),ProjectSettings.get_setting("application/config/name")])
