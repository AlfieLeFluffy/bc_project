...
func _unhandled_input(event: InputEvent) -> void:
	if event.is_action_pressed("add_to_board") and arguments.has_all(["name","text"]):
		Signals.emit_signal('create_board_element',ElementResource.new().setup(ElementResource.elementType.FILE,arguments["name"],computer.interactableResource.timeline,arguments["text"],null))
