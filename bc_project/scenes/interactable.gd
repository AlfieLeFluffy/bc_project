...
func _ready() -> void:
	...
	$LabelBG/InteractKeyBG/Label.text = InputMap.action_get_events("interact")[0].as_text()[0].to_upper()
	
	$LabelBG/Label.text = get_meta("Name")
...
func _process(delta: float) -> void:
	if $LabelBG.visible == true and $LabelBG/InteractKeyBG.visible == true:
		IsActive = true
	else:
		IsActive = false
...
func _on_area_entered(area: Area2D) -> void:
	if area.name == "interactRadius":
		$LabelBG/InteractKeyBG.visible = true

func _on_area_exited(area: Area2D) -> void:
	if area.name == "interactRadius":
		$LabelBG/InteractKeyBG.visible = false
